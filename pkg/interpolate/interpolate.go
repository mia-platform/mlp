...
func checkEnvs(envs map[string]envVar, prefixes []string) error {
	for env := range envs {
		...
		envValue, err := getEnv(env, prefixes)
		if err != nil {
			return err
		}
		...
		envs[env] = envVar{name: envs[env].name, value: envValue}
	}
	return nil
}
...
func getEnv(env string, prefixes []string) (string, error) {
	for _, prefix := range prefixes {
		varPrefixed := prefix + env
		if val, exists := os.LookupEnv(varPrefixed); exists {
			val = strings.ReplaceAll(val, `"`, asciiDoubleQuote)
...
func interpolateVariables(file string, envs map[string]envVar) []byte {
	for env := range envs {
		...
		// If the env value contains an escaped double quote (\"), this value will be escaped
		// resulting (\\"). So, we remove added escape preserving the original escaped quote.
		envValue = strings.ReplaceAll(envValue, fmt.Sprintf(`\\%s`, asciiDoubleQuote), `\"`)
