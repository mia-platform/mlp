...
func (f *Flags) AddFlags(set *pflag.FlagSet) {
	set.StringSliceVarP(&f.prefixes, prefixesFlagName, prefixesFlagShort, nil, prefixesFlagUsage)
	set.StringSliceVarP(&f.inputPaths, inputFlagName, inputFlagShort, nil, inputFlagUsage)
	set.StringVarP(&f.outputPath, outputFlagName, outputFlagShort, "interpolated-files", outputFlagUsage)
}
...
func (o *Options) Validate() error {
	...
	if len(o.inputPaths) > 1 && slices.Contains(o.inputPaths, stdinToken) {
		return fmt.Errorf("cannot read from stdin and path together")
	}
...
func (o *Options) Run(context.Context) error {
	...
	for _, path := range pathsToInterpolate {
		...
		interpolatedData, err := o.interpolate(data, name)
		if err != nil {
			return err
		}
...
func (o *Options) interpolate(data []byte, name string) ([]byte, error) {
	...
	for _, env := range o.envNamesToInterpolate(data) {
		noQuoteTemplateFuncs[env] = func() (string, error) {
			return o.substituteEnv(env, func(str string) string {
...
func (o *Options) envNamesToInterpolate(data []byte) []string {
...
func (o *Options) substituteEnv(envName string, quotingStringFun func(string) string) (string, error) {
	envsToCheck := make([]string, 0, len(o.prefixes)+1)
